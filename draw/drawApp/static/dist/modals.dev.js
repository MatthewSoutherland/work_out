...
function drawLineModalFunction() {
  ...
  dragElement(document.getElementById("".concat(activeModal, "-modal-content")));
...
function saveJson() {
  var fileName = getElementValue("save-json-filename");
...
function loadJson() {
  var fileName = getElementValue("existing-files");
  ...
  fetch("/load-json/".concat(fileName), {
    method: "GET"
  }) // Change to 'GET' if you update the Flask route to accept 'GET'
  .then(function (response) {
    return response.json();
  }).then(function (data) {
    console.log("Success:", data);
...
function showShapesModalFunction() {
  ...
  var viewer = document.getElementById("show-shapes-viewer");
  viewer.innerHTML = "";
  ...
  for (var key in shapes) {
    ...
    for (var attr in shape) {
      ...
      var input = createElement("input", {
        ...
        id: "".concat(key, "-").concat(attr),
        style: "font-weight: ".concat(fontWeight),
        value: shape[attr]
      });
    ...
    var editButton = createElement("button", {
      ...
      onclick: "editShapeModalFunction(\"".concat(key, "\")")
    }, "EDIT");
    var deleteButton = createElement("button", {
      "class": "delete-button insert-button modal-button",
      onclick: "deleteShapeModalFunction(\"".concat(key, "\")")
    }, "DELETE");
...
function editShapeModalFunction(key) {
  ...
  for (var attr in shape) {
    var element = document.getElementById("".concat(key, "-").concat(attr));
...
function dragElement(elmnt) {
  ...
  if (document.getElementById("".concat(activeModal, "-modal-header"))) {
    // if present, the header is where you move the DIV from:
    document.getElementById("".concat(activeModal, "-modal-header")).onmousedown = dragMouseDown;
  } else {
